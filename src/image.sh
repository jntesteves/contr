...
NS__get_label() {
	command podman image inspect --format "{{index .Config.Labels \"${2}\"}}" "$1"
}
NS__pull_if_missing() {
	command podman image exists "$1" && :
	NS__status=$?
	if [ "$NS__status" -eq 125 ]; then
		...
		command podman image pull "$1" || abort "Image '${1}' does not exist and Podman failed to pull a new image"
	fi
}
